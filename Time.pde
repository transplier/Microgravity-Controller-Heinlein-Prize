...
boolean CheckForInterrupted() {
  byte statusByte = EEPROM.read(EEPROM_STATUS);
  ...
  if(statusByte & EEPROM_STATUS_TIME_VALID) {
    ...
    b1 = EEPROM.read(EEPROM_TIME_1);
    b2 = EEPROM.read(EEPROM_TIME_2);
    b3 = EEPROM.read(EEPROM_TIME_3);
    b4 = EEPROM.read(EEPROM_TIME_4);
    if((b1 ^ b2 ^ b3 ^ b4) == EEPROM.read(EEPROM_TIME_CHECK)) {
      time_offset = b1;
      time_offset |= (b2<<8);
      time_offset |= (b3<<16);
      time_offset |= (b4<<24);
      DEBUG("CheckForInterrupted: Time signature: ");
      DEBUGF(time_offset, BIN);
      DEBUG("\n");
      return true;
    } else {
...
void WriteTime() {
  ...
  b1 = time_offset & 0x000000FF;
  b2 = (time_offset & 0x0000FF00) >> 8;
  b3 = (time_offset & 0x00FF0000) >> 16;
  b4 = (time_offset & 0xFF000000) >> 24;
  c = b1 ^ b2 ^ b3 ^ b4;
  EEPROM.write(EEPROM_STATUS, statusByte);
  EEPROM.write(EEPROM_TIME_1, b1);
  EEPROM.write(EEPROM_TIME_2, b2);
  EEPROM.write(EEPROM_TIME_3, b3);
  EEPROM.write(EEPROM_TIME_4, b4);
  EEPROM.write(EEPROM_TIME_CHECK, c);
  DEBUG("WriteTime: done");
}
